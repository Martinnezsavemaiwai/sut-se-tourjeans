...
import { CreateHotelImage, GetHotelByID, UpdateHotelByID } from "../../../services/http";
...
function HotelEdit() {
    ...
    const [hotels, setHotels] = useState<HotelsInterface | null>(null);
  ...
  const fetchAccommodationsData = async () => {
    ...
    const onFinished = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const resHotel = await UpdateHotelByID(HotelsData,hotels?.ID ?? 0);
    ...
    useEffect(() => {
        if (hotels) {
            setSelectedHotel(hotels.HotelName ?? "");
            setImage(hotels.HotelImagesInterface);}
      }, [hotels]);
