...
import { GetVehicleTypes, CreateVehicle, CreateVehicleImage } from "../../../services/http";
...
function TransportCreate() {
    ...
    const [images, setImage] = useState<VehicleImagesInterface[]>([]);
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (event.target.files) {
            ...
            if (VehicleNameError) {
                message.error(VehicleNameError);
                return;
            }
    ...
    const onFinished = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!vehicleName || !selectedType || !images || fileList.length === 0) {
            message.error("กรุณากรอกข้อมูลให้คร้บถ้วน");
            return;
        }

        if (!validateVehicleName(vehicleName)) return;
        if (!selectedType || isNaN(Number(selectedType))) {
            message.error("กรุณาเลือกประเภทยานพาหนะ");
            return;
        }
        ...
        const vehicleData: VehiclesInterface = {
        ...
        if (res && resVehicleImages) {
            message.success("สร้างการขนส่งสำเร็จ", 3);
            setVehicleName("");
            setSelectedType("");
            setImage([]);
            setFileList([]);
        } else {
            message.error("เกิดข้อผิดพลาดในการบันทึกข้อมูล", 3);
        }
    ...
    return (
        <div className="transport-create-page-container">
            <Navbar page={"trnsport-management"} />
            <div className="transport-create-page">
