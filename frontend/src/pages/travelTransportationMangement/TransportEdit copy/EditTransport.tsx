...
import {  GetVehicleTypes, UpdateVehicleByID, CreateVehicleImage, GetVehicleByID } from "../../../services/http";
...
function EditTransport() {
  ...
  const [vehicles, setvehicles] = useState<VehiclesInterface | null>(null);
  ...
  useEffect(() => {
    if (vehicles) {
      ...
      setSelectedType(vehicles?.VehicleTypeID?.toString() || "");
      setImages(vehicles.VehicleImagesInterface || []);
    }
  }, [vehicles]);
  ...
  const onFinished = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await UpdateVehicleByID(updatedVehicle, vehicles?.ID || 0);
    ...
    const resVehicleImages = await CreateVehicleImage(formDataImageVehicle, res?.data?.ID || 0);
